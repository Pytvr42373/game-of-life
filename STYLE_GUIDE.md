...
## 1. 主题切换（Theme Toggle）
- **适用范围**：主页 `/`、康威 `/life/`、1A2B `/1A2B/`、Stroop `/stroop/`、点灯 `/light-up/`、光线侦探 `/ray-lab/`、好玩游戏频道 `/action-games/`。
...
- **按钮语义**：按钮展示的是**点击后将切换到的目标主题**，不是当前主题。当前为 4399 时使用暗色背景与 arcade 图标；当前为 arcade 时必须使用亮色背景与 4399 图标，图标和底色都要清楚表达即将进入亮色主题。
- **默认图标**：**不用 emoji**，自绘内联 SVG（素材库无现成，自主绘制）：
  ...
  - 图标颜色跟随目标主题，必须与按钮背景形成清晰对比。
- **好玩游戏频道例外**：`/action-games/` 保留频道自有图标，不使用世界树 / 吃豆人。目标 arcade 使用机甲六边形；目标 4399 使用清风叶片。当前处于 arcade 时，切换按钮使用亮米白背景与清晰绿色的清风叶片图标。
